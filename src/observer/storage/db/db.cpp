...
RC Db::create_table(const char *table_name,
  ...
  std::string table_file_path =
      table_meta_file(path_.c_str(), table_name);  // 表元数据文件路径
  Table *table = new Table();                      // 创建表对象
  int32_t table_id = next_table_id_++;             // 生成表ID

  // 创建表
  rc = table->create(this, table_id, table_file_path.c_str(), table_name,
                     path_.c_str(), attributes);
...
RC Db::flush_meta() {
  ...
  if (n < 0) {
    ...
    LOG_ERROR(
        "Failed to write db meta file. db=%s, file=%s, buffer size=%ld, write "
        "size=%d",
    ...
    if (ec) {
      ...
      LOG_INFO(
          "Successfully write db meta file. db=%s, file=%s, "
          "check_point_lsn=%ld",
